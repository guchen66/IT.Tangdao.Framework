...
namespace IT.Tangdao.Framework.Events
{
    public class EventAggregator : IEventAggregator
    {
        ...
        /// Value: 对应类型的空事件对象
        /// </summary>
        private static readonly ConcurrentDictionary<Type, EventBase> _emptyEvents = new ConcurrentDictionary<Type, EventBase>();
        ...
        public void Subscribe<T>(DaoEventHandler<T> handler) where T : EventBase, new()
        {
            AddHandler(handler, false);
        }

        public void SubscribeAsync<T>(DaoAsyncEventHandler<T> handler) where T : EventBase, new()
        {
        ...
        public void Publish<T>(T @event = null) where T : EventBase, new()
        {
        ...
        /// 异步发布事件
        /// </summary>
        public async Task PublishAsync<T>(T @event = null) where T : EventBase, new()
        {
            var eventObj = @event ?? GetEmptyEvent<T>();
        ...
        /// 取消订阅同步事件
        /// </summary>
        public void UnSubscribe<T>(DaoEventHandler<T> handler) where T : EventBase, new()
        {
        ...
        /// 取消订阅异步事件
        /// </summary>
        public void UnSubscribeAsync<T>(DaoAsyncEventHandler<T> handler) where T : EventBase, new()
        {
            if (handler == null) return;
        ...
        private static T GetEmptyEvent<T>() where T : EventBase, new()
        {
            // 从缓存中获取或创建空事件对象
