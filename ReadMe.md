...
```C#
 public IEventAggregator _daoEventAggregator;
...
#### 20、观察者模式事件通知TangdaoMessenger

当时有内部框架的IOC时，不需要外部委托SetResolver，当使用其他的IOC时需要通过SetResolver来解析

```C#
TangdaoMessenger.SetResolver(reg => Container.Get(reg.RegisterType) as INoticeObserver);
...
```C#
TangdaoMessenger.Instance.ChainRegister(typeof(LightViewModel), typeof(ElectViewModel), typeof(PressureViewModel));
```

2、

````C#
TangdaoMessenger.Instance.ChainRegister().Add(typeof(LightViewModel)).Add(typeof(ElectViewModel)).Add(typeof(PressureViewModel));
````
...
```C#
 public class ElectViewModel : BaseViewModel, IMessageObserver
 {
     private string _tag;
...
```C#
 ...
 Context.Tag = "数据改变了";
 TangdaoMessenger.Instance.NotifyAllObservers(Context);
```
...
```C#
 TangdaoMessenger.Instance.Unregister("Key");
```

注销所有通知：

```C#
TangdaoMessenger.Instance.Unregister();
```
...
| 旧项                                | 新项                              |
| :---------------------------------- | :-------------------------------- |
| `DaoViewModelBase`                  | `ViewModelBase`                   |
| `IDaoEventAggregator`               | `IEventAggregator`                |
| `DaoEventBase`                      | `EventBase`                       |
| `IT.Tangdao.Framework.DaoExtension` | `IT.Tangdao.Framework.Extensions` |
| `IT.Tangdao.Framework.EventArs`     | `IT.Tangdao.Framework.Events`     |
| `IT.Tangdao.Core.DaoExtension`      | `IT.Tangdao.Core.Extensions`      |
| `IT.Tangdao.Core.EventArs`          | `IT.Tangdao.Core.Events`          |
